...
class TorrentDownload(QObject):
	...
	def info_hash(self):
		s = self.handle.status()
		#self.handle.info_hash().to_string()
	...
	'''
	def select_file(self, select_path, time = 60):
		...
		if len(select_path) == 0 or select_path == "/":
			self.resume()
		...
		for i, path in enumerate(files):
			if path == select_path:
				schedule_file(i, file)
				return
			
			if path.startwith(select_path):
				schedule_file(i, file)
